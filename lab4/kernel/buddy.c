...
static inline void __push_page(page_t **head, page_t *page) { // O(1)
    if (*head == NULL) {
        *head = page;
        return;
    }
...
static inline page_t *__pop_page(page_t **head) { // O(1)
    if (*head == NULL) {
        return NULL;
    }
    page_t *page = *head;
...
static inline void __remove_page(page_t **head, page_t *page) { // O(1)
    if (page->prev) {
...
void buddy_init(phys_addr_t start, phys_addr_t end) { // O(number of pages)
...
page_t *alloc_pages(unsigned long order) { // O(MAX_ORDER)
...
void free_pages(page_t *page, unsigned long order) { // O(MAX_ORDER)
    int cur_order = order;
...
phys_addr_t get_addr_by_page(page_t *page) { // O(1)
...
page_t *get_page_by_addr(phys_addr_t addr) { // O(1)
        ...
        while(page != NULL) {
            ...
            if(cur_order == 0) {
                if(!(page_end < start || page_addr > end)) {
                    print_string("\nPage partially or fully reserved\n");
